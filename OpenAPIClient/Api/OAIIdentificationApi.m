#import "OAIIdentificationApi.h"
#import "OAIQueryParamCollection.h"
#import "OAIApiClient.h"


@interface OAIIdentificationApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation OAIIdentificationApi

NSString* kOAIIdentificationApiErrorDomain = @"OAIIdentificationApiErrorDomain";
NSInteger kOAIIdentificationApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) identificationLoginMonitorGetWithCompletionHandler: 
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/identification/login/monitor"];
...
-(NSURLSessionTask*) identificationLoginReturnGetWithCode: (NSString*) code
    ...
    if (code == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kOAIIdentificationApiErrorDomain code:kOAIIdentificationApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }
    ...
    if (cookie == nil) {
        ...
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"cookie"] };
            NSError* error = [NSError errorWithDomain:kOAIIdentificationApiErrorDomain code:kOAIIdentificationApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    if (cookie2 == nil) {
        NSParameterAssert(cookie2);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"cookie2"] };
            NSError* error = [NSError errorWithDomain:kOAIIdentificationApiErrorDomain code:kOAIIdentificationApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/identification/login/return"];
    ...
    if (cookie2 != nil) {
        headerParams[@"cookie"] = cookie2;
    }
    // HTTP header `Accept`
    NSString *acceptHeader = [self.apiClient.sanitizer selectHeaderAccept:@[@"text/html;charset=utf-8"]];
    if(acceptHeader.length > 0) {
        headerParams[@"Accept"] = acceptHeader;
    }
...
-(NSURLSessionTask*) identificationUserStampUuidPostWithUuid: (NSString*) uuid
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/identification/user/stamp/{uuid}"];
